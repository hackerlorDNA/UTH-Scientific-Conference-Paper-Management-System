...
export const AuthorDashboard: React.FC<DashboardProps> = ({ onNavigate, onViewPaper }) => {
  ...
  const getStatusBadge = (status: string) => {
      switch (status) {
          case 'submitted':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">Submitted</span>;
          case 'under_review':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700">Under Review</span>;
          case 'accepted':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">Accepted</span>;
          case 'rejected':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">Rejected</span>;
          case 'revision_required':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-700">Revision Required</span>;
          case 'withdrawn':
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-700">Withdrawn</span>;
          default:
              return <span className="inline-flex px-2 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-700">{status}</span>;
      }
  };
    ...
    <div className="w-full bg-background-light dark:bg-background-dark py-8 px-5 md:px-10 flex justify-center">
        <div className="w-full max-w-[1200px]">
            ...
            <div className="bg-white dark:bg-card-dark rounded-xl border border-border-light shadow-sm overflow-hidden">
                {loading ? (
                    <div className="p-8 text-center text-text-sec-light">Đang tải dữ liệu...</div>
                ) : (
                    <>
                                    ...
                                    {submissions.length > 0 ? (
                                        submissions.map((sub) => (
                                            <tr key={sub.id} className="hover:bg-background-light dark:hover:bg-gray-800 transition-colors">
                                                ...
                                                <td className="p-4 text-text-sec-light">{new Date(sub.submissionDate).toLocaleDateString()}</td>
                                                <td className="p-4 flex items-center">
                                                    ...
                                                    {sub.status !== 'withdrawn' && sub.status !== 'accepted' && sub.status !== 'rejected' && (
                                                        <button 
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-8 text-center text-text-sec-light">Chưa có bài báo nào được nộp.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
